...
st.set_page_config(page_title="Personal Auto UW & Pricing", layout="wide")
...
st.title("🚗 US Personal Auto — Underwriting & Pricing Prototype")
st.caption("Model-backed: XGBoost (frequency) · Gamma GLM (severity) · LightGBM (bind propensity) "
           "+ deterministic eligibility/premium rules engine.")
...
with tab_dashboard:
    ...
    k1.metric("Submissions", f"{len(filtered):,}")
    k2.metric("Avg Composite Score", f"{filtered['composite_score'].mean():.1f}" if len(filtered) else "-")
    stp_rate = (filtered["recommended_action"] == "Auto-Quote / STP").mean() if len(filtered) else 0
    k3.metric("STP Rate", f"{stp_rate:.1%}")
    decline_rate = (filtered["uw_risk_band"] == "Decline").mean() if len(filtered) else 0
    k4.metric("Decline Rate", f"{decline_rate:.1%}")
    k5.metric("Avg Quoted Premium", f"${filtered['final_quoted_premium'].mean():,.0f}" if len(filtered) else "-")
    k6.metric("Avg Loss Ratio (current-rate)", f"{filtered['expected_loss_ratio'].mean():.1%}" if len(filtered) else "-")
    ...
    with col_a:
        ...
        fig = px.bar(band_counts, x="Band", y="Count", color="Band",
                     category_orders={"Band": ["Preferred", "Standard", "Non-Standard", "Decline", "Hard Stop"]})
    ...
    with col_c:
        st.markdown("**Avg Composite Score by State**")
        by_state = filtered.groupby("State")["composite_score"].mean().reset_index().sort_values(
            "composite_score", ascending=False)
        fig3 = px.bar(by_state, x="State", y="composite_score")
        st.plotly_chart(fig3, use_container_width=True)
    with col_d:
        st.markdown("**Technical vs Final Quoted Premium (sample)**")
        sample = filtered.sample(min(500, len(filtered)), random_state=1) if len(filtered) else filtered
        fig4 = px.scatter(sample, x="technical_premium", y="final_quoted_premium",
                           color="uw_risk_band", opacity=0.6,
                           labels={"technical_premium": "Technical Premium ($)",
                                   "final_quoted_premium": "Final Quoted Premium ($)"})
        fig4.add_shape(type="line", x0=0, y0=0, x1=16000, y1=16000, line=dict(dash="dash", color="gray"))
        st.plotly_chart(fig4, use_container_width=True)

    st.divider()
    st.markdown("**Rate Adequacy Exceptions** (Final Quoted Premium below Technical Premium)")
    exceptions = filtered[filtered["rate_adequacy_flag"] == True]
    st.write(f"{len(exceptions):,} of {len(filtered):,} submissions ({len(exceptions)/max(len(filtered),1):.1%}) flagged")
    show_cols = ["Policy_ID", "State", "uw_risk_band", "technical_premium", "final_quoted_premium",
                 "composite_score", "recommended_action"]
    st.dataframe(exceptions[show_cols].head(50), use_container_width=True)

# ============================================================================
# INTERFACE TAB (live what-if simulator)
# ============================================================================
with tab_interface:
    st.subheader("Live Underwriting & Pricing Simulator")
    st.caption("Enter a hypothetical submission. Frequency, severity, and bind probability come from the "
               "trained models; eligibility and pricing use the deterministic rules engine.")
    ...
    if submitted:
        ...
        st.divider()
        band_colors = {"Preferred": "green", "Standard": "blue", "Non-Standard": "orange",
                       "Decline": "red", "Hard Stop": "red"}
        st.markdown(f"### Result: :{band_colors.get(result.uw_risk_band,'gray')}[{result.uw_risk_band}]")
